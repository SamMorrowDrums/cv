...
With current implementations, CLIs are can be a genuinely a better experience than MCP servers sometimes. Popular CLIs are already in the training set. Bash is absurdly powerful - streaming, parsing, formatting, networking, SSH. Developers never really move away from the shell because the composability is unmatched. When [Mario Zechner](https://mariozechner.at/) says he can replace 21 MCP tools with four bash scripts, he's not wrong about the efficiency.
...
- **False Economy**: if the model tries to find things and fails a lot, or invokes things and goes through lots of iterations getting it right - it is possible to use more rather than less tokens. For very narrow agents specific instructions and up-front tool declarations can be more efficient.
