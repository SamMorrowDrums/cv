...
function generateExcerpt(content, maxLength = 150) {
  ...
  // Remove HTML tags, markdown headers, links, and other formatting
...
export function getAllBlogPosts(asOf = new Date()) {
  const publishThrough = new Date(asOf);
  publishThrough.setUTCHours(23, 59, 59, 999);

  return getAllDocuments(blogDirectory, blogSort).filter((post) => {
    if (!post.date) return true;
    return new Date(post.date) <= publishThrough;
  });
}

export function getPublishedBlogPostSlugs(asOf = new Date()) {
  return getAllBlogPosts(asOf).map((post) => post.slug);
}
