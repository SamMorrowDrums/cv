...
export function getBlogPostMeta(post, baseUrl = 'https://sam-morrow.com') {
  ...
  const author = post.author || 'Sam Morrow';
  const publishedTime = post.date;
  
  // Always use the generated OG image (which may be derived from content image or SVG fallback)
  const image = generateFallbackImage(title, baseUrl, post.slug);
